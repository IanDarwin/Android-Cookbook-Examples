...
. Prerequisites: Java 7 SDK, git, Maven
...
(Include android:install and android:run in the ante-penultimate step only if you actually want to run them all, and have a device or emulator with some good free space; else just do 'package')
...
| link:https://github.com/IanDarwin/Android-Cookbook-Examples/tree/master/JavaTimeDemo[JavaTimeDemo] | GS | Demos of java.time date/time API | Actually JSR-310 API; almost same but different package
